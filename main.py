...
if __name__ == "__main__":
    uvicorn.run("main:app", host=apiip, port=5000, log_level="info")
@app.post("/")
async def add_item(request: dict):
    if request["auth"] in authlist:
        ...
        return {"status": "oka", "message": "Successfully authenticated!"}
    else:
...
gamers = []
lastorsince = "Something broke lol"
...
async def status():
    global statusname, statuscolour, statusemoji, online_time, lastorsince
    for index, uuid in enumerate(uuid_list):
        ...
        if debug:
            ...
            logging.debug(online_status[index])
            logging.debug(online_list[index])
            logging.debug(username)
            logging.debug(last_online[index])
            logging.debug(timestamper(current_time-last_online[index]))
...
async def info(ctx):
    global statusStatus, skillIssue, fortnitestatus, mayorstatus
...
async def mayorchannel():
    parse_mayorapi = mayorapi()
    global mayorchannelid, currentmayorthumbnail, mayorruncount
